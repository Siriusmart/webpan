        ...
        for (let [filePath, difftype] of outputDiff.entries()) {
            if (writeEntries.has(filePath)) {
                ...
                if (previousWriter?.content !== "remove") {
                    console.warn(
                        `${this.getIdent().join("#")} is trying to write to ${filePath}, but it is already modified by ${previousWriter?.processor.meta.childPath}#${previousWriter?.processor.meta.procName}!`
                    );
                }
            }
