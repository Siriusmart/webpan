...
function calcDiffByEq<K, V>(before: Map<K, V>, after: Map<K, V>, eq: (a: V, b: V) => boolean): procEntries.DiffEntries<K> {
...
function calcDiffByExtractor<K, V, T>(before: Map<K, V>, after: Map<K, V>, extractor: (a: V) => T): procEntries.DiffEntries<K> {
    return calcDiffByEq(before, after, (a, b) => extractor(a) === extractor(b));
}

function calcDiff<K, V>(before: Map<K, V>, after: Map<K, V>): procEntries.DiffEntries<K> {
